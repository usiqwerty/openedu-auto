...
Контент курсов поделён на блоки. Поробнее можно почитать по
ссылке: https://docs.openedx.org/en/latest/educators/how-tos/advanced_features/lti_determine_content_address.html
...
| vertical                                          | юнит, тема                                          |
| problem, html, video, library_content, discussion | единица контента, таких может быть несколько в теме |

Мне не совсем понятно назначение `library_content`, такой блок в стречался на
странице "правила обучения на курсе", которую нужно пройти для получения доступа к материалам курса.
Внутри `library_content` были обычные задачи `problem`.
...
| unified-match         | таблица                                                                               |
| select                | выпадающий список                                                                     |
| fill                  | ввод значения текстом                                                                 |
| fixed-match (устарел) | таблица установки соответсвия (первый столбец зафиксирован). Заменён на unified-match |
| freematch (устарел)   | таблица соответсвия без фиксированных столбцов. Заменён на unified-match              |
| new_match (устарел)   | продвинутая таблица соответсвия. Заменён на unified-match                             |

И мы должны уметь парсить эти вопросы.
В результате для каждого vertical блока мы получаем список задача (`list[list[Question]]`, т.к. задача это
`list[Question]`)

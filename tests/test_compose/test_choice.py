...
def test_success_single():
    ...
    assert compose_answer(ans, ids, options) == ("input_bibaboba_0_1", "choice_1")
...
def test_no_solution_single():
    ...
    with pytest.raises(NoSolutionFoundError):
        compose_answer(ans, ids, options)
...
def test_success_multiple():
    ...
    assert compose_answer(ans, ids, options) == ("input_bibaboba_0_1[]", ["choice_1", "choice_2"])
